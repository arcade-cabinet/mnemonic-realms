...
export class TrainingGroundEvent extends RpgEvent {
  ...
  async startScene(player: RpgPlayer) {
    ...
    // 1. Spawn Lira (npc_lira) at a suitable position near the player for dialogue
    // Assuming Lira is already in the party, this is more about her "presence" for the scene.
    // If Lira is a party member, she might already be following.
    // For dialogue, we might want a dedicated NPC instance for the scene.
    const liraEvent = await createDynamicEvent({
      map: player.map,
      x: player.position.x + 1, // Place Lira next to the player
      y: player.position.y,
      event: {
        name: 'Lira_Scene4_NPC',
        graphic: 'npc_lira',
        speed: 1,
    ...
    // Ensure Lira is facing the player
    if (liraEvent) {
      liraEvent.event.setDirection(player.direction); // Lira faces player
      player.setDirection(liraEvent.event.direction); // Player faces Lira
    }
    ...
    // Lira's in-combat dialogue is handled by the combat system itself, not here.
    // The combat system would check if Lira is in the party and trigger her specific lines.
    await player.showText(
    ...
    await player.showText("Not bad at all. You're a natural.", { speaker: 'Lira' });
    ...
    // Part B: Remix and Broadcast Tutorial (Scene transitions back to Lira's Workshop)
    await player.showText(
      'Now for the real work. Come back to the Workshop — I want to show you the Remix Table.',
      { speaker: 'Lira' },
    );

    // Teleport player and Lira to the workshop (assuming workshop is at 9,19 in Village Hub)
    await player.changeMap('village-hub', { x: 9, y: 19 });
    if (liraEvent) {
      await liraEvent.event.changeMap('village-hub', { x: 10, y: 19 }); // Lira next to player in workshop
    }
    ...
    await player.showText('One more thing. The most important thing.', { speaker: 'Lira' });
    ...
    // Player broadcasts MF-02: Lira's Warmth
    // This would involve a player action, likely via a GUI or specific interaction.
    // For the tutorial, we can script it.
    player.removeItem('MF-02', 1); // Consume Lira's Warmth fragment
    await player.call('vibrancy-change', { zone: 'village-hub', delta: 12 }); // Apply vibrancy change
    await player.showText(
      'SYSTEM: You broadcast "Lira\'s Warmth" into the workshop. Vibrancy in Village Hub increased by +12! (potency 3 × 3 = 9, +3 emotion match [Joy resonates with Village Hub]) The world remembers what you shared.',
      { speaker: 'SYSTEM', type: 'dialog' },
    );
    await player.showText(
