...
export class StagnationClearingEvent extends RpgEvent {
    ...
    async onSceneTrigger(player: RpgPlayer, map: RpgMap) {
        ...
        await player.showText("This is a Stagnation Zone. Something — someone — froze this patch of the world. Time stopped here. Change stopped.", { speaker: lira });

        // Player enters the clearing (implied by the trigger, no actual movement needed here)
        await player.showText("Look at the butterflies. Perfect. Every wing-scale, every spot of color. Beautiful, isn't it?", { speaker: lira });
        await player.showText("...", { speaker: lira });
        await player.showText("But they'll never land. They'll never fly anywhere new. They're just... frozen. Forever.", { speaker: lira });
