import { EventData, RpgCommonMap, RpgCommonPlayer, RpgEvent, RpgMap, type RpgPlayer } from '@rpgjs/server';
...
export class EndgameBloomEvent extends RpgEvent {
    ...
    async onChanges(player: RpgPlayer) {
        ...
        if (player.getVariable('worlds-new-dawn-broadcast') && !player.getVariable('endgame-bloom-triggered')) {
    ...
    private async triggerEndgameBloom(player: RpgPlayer) {
        ...
        // 3. Dialogue sequence
        await player.showText('The world blooms. Vibrancy 95 in every zone. The Endgame Bloom has transformed the Unfinished World into something new — not finished, but alive. Not perfect, but growing.', {
            talkWith: this
        });
