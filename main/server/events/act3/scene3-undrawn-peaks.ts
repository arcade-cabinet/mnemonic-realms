...
export default class UndrawnPeaksScene3Event extends RpgEvent {
  async onInit(player: RpgPlayer, map: RpgMap) {
    ...
    const liraEvent = map.createDynamicEvent({
      ...
      y: player.y,
      event: {
        name: 'npc_hana_undrawn_peaks_scene',
        graphic: 'npc_hana',
        onInit(eventPlayer: RpgPlayer) {
          eventPlayer.setGraphic('npc_hana');
        },
      },
    });
    ...
    const callumEvent = map.createDynamicEvent({
      ...
      y: player.y,
      event: {
        name: 'npc_artun_undrawn_peaks_scene',
        graphic: 'npc_artun',
        onInit(eventPlayer: RpgPlayer) {
          eventPlayer.setGraphic('npc_artun');
        },
      },
    });
    ...
    const aricEvent = map.createDynamicEvent({
      ...
      y: 35,
      event: {
        name: 'npc_janik_fortress_gate',
        graphic: 'npc_janik',
        onInit(eventPlayer: RpgPlayer) {
          eventPlayer.setGraphic('npc_janik');
        },
      },
    });
    ...
    await player.showText('Janik steps aside. The path to the Fortress is open.', {
      type: 'system',
    });
