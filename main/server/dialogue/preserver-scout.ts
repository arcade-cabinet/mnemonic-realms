...
export default async function dialogue(player: RpgPlayer, event: RpgEvent) {
  ...
  // Lira's question
  await player.showText('Who are you?', { speaker: 'lira' });
  ...
  // Lira's response to the preservation explanation
  ...
  // Lira's internal monologue/summary after the encounter.
  await player.showText(
