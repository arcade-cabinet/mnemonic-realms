# Ralph Agent Instructions - Mnemonic Realms
...
2. `.kiro/steering/structure.md` — project structure conventions (if exists)
3. `.kiro/steering/tech.md` — tech stack and tooling (if exists)
...
1. Find the lowest-numbered **top-level** task in `tasks.md` that is NOT marked with `[X]` or `[x]`. A top-level task is one at the root indentation level (e.g., `- [ ] 1.`, `- [ ] 2.`). Subtasks nested under a top-level task (e.g., `1.1`, `1.2`) are NOT independent tasks — they are part of their parent and will be implemented together with it.
...
- Module/package organization (barrel exports, index files, etc.)

**Language & Type System**
- Preferred language idioms (e.g., guard clauses vs nested ifs, early returns)
- Type annotation style (interfaces vs types in TS, type hints in Python)
- Null/optional handling (Optional<T>, Maybe, nullable types)
- Async patterns (async/await, Promises)

**Error Handling**
- Custom error classes/types and hierarchy
- How errors propagate (thrown exceptions, Result types, error codes)
...
- Test setup/teardown patterns (fixtures, factories, mocks)
- Assertion style (expect, assert, should)
- How to run tests (commands, flags, environment variables needed)

**Build & Tooling**
- Build commands and flags that work
- Environment-specific gotchas (env vars, feature flags)
- Linting/formatting commands and any suppressions in use

## Stop Condition

After completing your one task, check if ALL tasks in `tasks.md` are marked `[x]` or `[X]`.

If all tasks are complete:

1. Build an HTML page at `.kiro/specs/SPECS_NAME/summary.html` that presents a readable, multi-pane summary of the entire spec implementation. The page should be a single self-contained HTML file (inline CSS and JS, no external dependencies). Structure it as follows:

   **Page title:**
   - Use "Ralph Loop for Kiro Specs - Mnemonic Realms" as the static page title (in `<title>` and as the main heading).

   **Top pane — Global Summary:**
   - Display the spec name (SPECS_NAME) prominently as a field in this pane.
   - Show a visual status indicator: green if all tasks are marked `[x]` or `[X]` (fully implemented), red if any tasks are marked `[F]` or unchecked (partial/failed).
   ...
   **Left pane — Task Tree:**
   ...
   - Use visual indicators for task status: `[x]` or `[X]` = completed, `[F]` = failed, unchecked = incomplete.

   **Right pane — Timing:**
   - Render the timing data from `specs_time.md` as an HTML table (task ID, start time, end time, elapsed time).

   **Styling:**
   - Clean, modern CSS inspired by https://kiro.dev aesthetic
   - Use a side-by-side layout (e.g., flexbox) with the task tree taking roughly 60% width and the timing table taking 40%.
